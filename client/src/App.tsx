...
import { User, Clock, CheckCircle, XCircle, Info, RotateCcw, HelpCircle, ChevronDown, LogOut, Printer, BookOpen, BarChart3, ChevronRight, AlertCircle, Timer, FileText, Maximize2, Minimize2 } from 'lucide-react';
...
const App = () => {
  ...
  const getStatusShape = (status, isSelected = false) => {
    const base = "w-9 h-8 flex items-center justify-center text-[12px] font-bold border transition-all ";
  ...
  const getCount = (status) => questions.filter(q => q.status === status).length;

  // --- SUB-VIEWS ---

  if (view === 'landing') {
    return (
      <div className="min-h-screen bg-[#f3f7f8] flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-white shadow-2xl rounded-lg overflow-hidden border">
          ...
          <div className="p-8 space-y-6">
            <div>
              ...
              <input type="text" value={examName} onChange={(e) => setExamName(e.target.value)} className="w-full p-3 border rounded-lg bg-gray-50 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Candidate Name</label>
              <input type="text" placeholder="Enter name" value={candidateName} onChange={(e) => setCandidateName(e.target.value)} className="w-full p-3 border rounded-lg bg-gray-50 outline-none" />
            </div>
            ...
            <button disabled={!candidateName} onClick={() => setView('instructions')} className="w-full bg-[#337ab7] hover:bg-blue-700 disabled:bg-gray-300 text-white font-bold py-4 rounded uppercase transition-all shadow-md">Login</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <div className="p-6 border-t bg-gray-50 flex flex-col items-center shadow-lg shrink-0">
           ...
           <button disabled={!declarationChecked} onClick={() => { if(!document.fullscreenElement) document.documentElement.requestFullscreen().catch(()=>{}); setView('exam'); }} className={`px-16 py-3 font-black uppercase tracking-widest rounded shadow-md transition-all ${declarationChecked ? 'bg-[#337ab7] text-white' : 'bg-gray-300 text-gray-500'}`}>I am ready to begin</button>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-[#f3f7f8] flex items-center justify-center p-4">
        <div className="bg-white shadow-2xl rounded-lg max-w-xl w-full border overflow-hidden">
          ...
          <div className="p-8">
            ...
            <button onClick={() => setView('evaluation')} className="w-full bg-[#337ab7] hover:bg-blue-700 text-white font-black py-4 rounded uppercase shadow-lg">Confirm and Evaluate</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col items-center p-8">
        <div className="w-full max-w-4xl bg-white shadow-xl border rounded overflow-hidden flex flex-col max-h-[85vh]">
                ...
                {attemptedQs.map(q => {
                  ...
                  return (
                    <tr key={q.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3"><button onClick={() => setEvaluationResults(p => ({...p, [idx]: true}))} className={`w-full py-2.5 rounded-lg transition-all ${evaluationResults[idx] === true ? 'bg-green-600 text-white shadow-md' : 'bg-gray-100 text-gray-300'}`}><CheckCircle size={20}/></button></td>
                      <td className="p-3"><button onClick={() => setEvaluationResults(p => ({...p, [idx]: false}))} className={`w-full py-2.5 rounded-lg transition-all ${evaluationResults[idx] === false ? 'bg-red-600 text-white shadow-md' : 'bg-gray-100 text-gray-300'}`}><XCircle size={20}/></button></td>
                    </tr>
                  )
                })}
              </tbody>
          ...
          <div className="p-6 border-t bg-gray-50 flex justify-between items-center">
             ...
             <button disabled={!allChecked} onClick={() => setView('result')} className={`px-12 py-3 rounded-lg font-black transition-all ${allChecked ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>GENERATE RESULT</button>
          </div>
        </div>
      </div>
    );
  ...
  if (view === 'result') {
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 md:p-12 overflow-y-auto">
        ...
        <div className="max-w-4xl mx-auto printable-area bg-white border shadow-2xl rounded-2xl overflow-hidden">
           <div className="bg-[#337ab7] p-8 text-white flex justify-between items-end">
             ...
             <button onClick={handlePrint} className="no-print bg-white/20 hover:bg-white/30 p-3 rounded-lg border border-white/30 font-black text-sm transition-all"><Printer size={18}/> PRINT RESULT</button>
           </div>
           <div className="p-10">
              ...
              <button onClick={() => window.location.reload()} className="no-print w-full bg-gray-900 text-white font-black py-4 rounded-xl shadow-lg hover:bg-black uppercase tracking-widest active:scale-95 transition-all">Restart Session</button>
           </div>
        </div>
      </div>
    );
      ...
      <div className="h-16 border-b flex justify-between items-center px-6 bg-white shrink-0 relative z-50">
        ...
        <div className="flex items-center gap-6">
           ...
           <div className="relative">
             <button onClick={() => setShowSubmitMenu(!showSubmitMenu)} className="flex items-center gap-3 border-l pl-6 py-2 group hover:bg-gray-50">
             ...
             {showSubmitMenu && (
               <div className="absolute right-0 top-full mt-1 w-60 bg-white border shadow-2xl rounded-lg overflow-hidden z-50">
                 <button onClick={toggleFullScreen} className="w-full flex items-center gap-3 px-5 py-4 text-gray-700 hover:bg-gray-50 font-black text-[11px] uppercase border-b">
                   {isFullScreen ? <Minimize2 size={18}/> : <Maximize2 size={18}/>}
                   {isFullScreen ? 'Exit Full Screen' : 'Enter Full Screen'}
                 </button>
                 <button onClick={handleSubmitExam} className="w-full flex items-center gap-3 px-5 py-5 text-red-600 hover:bg-red-50 font-black text-sm uppercase tracking-widest active:bg-red-100">
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 flex flex-col overflow-hidden border-r bg-[#f4f7f9]">
          <div className="flex border-b bg-white shrink-0 overflow-x-auto no-scrollbar">
            {SECTIONS.map(sec => (
              <button key={sec.id} onClick={() => { setActiveTab(sec.id); setCurrentIndex(sec.start - 1); }} className={`px-7 py-4 text-[11px] font-black uppercase transition-all border-r whitespace-nowrap ${activeTab === sec.id ? 'bg-[#337ab7] text-white shadow-inner' : 'text-gray-400 hover:bg-gray-50'}`}>
                {sec.label}
              </button>
            ))}
          </div>
                ...
                <div className="flex items-center gap-4">
                  {pdfUrl && (
                    <div className="flex items-center gap-1">
                      <button onClick={() => setPdfHeight(h => Math.max(200, h - 100))} className="p-1 hover:bg-gray-200 rounded transition-colors"><Minimize2 size={14}/></button>
                      <button onClick={() => setPdfHeight(h => Math.min(1200, h + 100))} className="p-1 hover:bg-gray-200 rounded transition-colors"><Maximize2 size={14}/></button>
                    </div>
                  )}
                    ...
                    <div className="max-w-xs">
                      ...
                      <div className="grid grid-cols-4 gap-3">
                        {['1','2','3','4','5','6','7','8','9','0','.','-'].map(k => (
                          <button key={k} onClick={() => { const u = [...questions]; u[currentIndex].numericValue += k; setQuestions(u); }} className="p-4 bg-white border-2 border-gray-100 font-black hover:bg-blue-600 hover:text-white rounded-xl text-lg shadow-sm transition-all hover:scale-105">{k}</button>
                        ))}
                        <button onClick={()=>{ const u = [...questions]; u[currentIndex].numericValue = u[currentIndex].numericValue.slice(0, -1); setQuestions(u); }} className="col-span-2 p-4 bg-red-50 text-red-600 font-black border-2 border-red-100 rounded-xl text-[10px] uppercase">Backspace</button>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="h-18 bg-white border-t flex items-center justify-between px-8 shrink-0 py-4 shadow-sm">
             <div className="flex gap-4">
               <button onClick={() => handleQuestionAction('MARK_REVIEW')} className="px-6 py-2.5 border-2 border-[#337ab7] text-[#337ab7] font-black text-[11px] rounded-lg hover:bg-blue-50 uppercase tracking-tighter transition-all">Mark for Review & Next</button>
               <button onClick={() => handleQuestionAction('CLEAR')} className="px-6 py-2.5 border-2 border-gray-200 text-gray-500 font-black text-[11px] rounded-lg hover:bg-gray-100 uppercase tracking-tighter transition-all">Clear Response</button>
             </div>
             <button onClick={() => handleQuestionAction('SAVE_NEXT')} className="px-14 py-3 bg-[#337ab7] text-white font-black text-sm rounded-xl hover:bg-blue-800 uppercase shadow-xl transition-all active:scale-95 tracking-widest">Save & Next</button>
          </div>
        </div>
